...
class avl_tree(object):
    ...
    def build_avl_tree(self, values):
        ...
        def build_tree_from_sorted_list(start, end):
            if start > end:
                return None

            mid = (start + end) // 2
            node = tree_node(values[mid])

            node.left = build_tree_from_sorted_list(start, mid - 1)
            node.right = build_tree_from_sorted_list(mid + 1, end)

            self.update_height(node)
            return node

        return build_tree_from_sorted_list(0, len(values) - 1)