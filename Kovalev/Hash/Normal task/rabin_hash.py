...
def count_continuous_occurrences(text, pattern):
    ...
    while i < len(occurrences):
        ...
        count = 1
        while i + 1 < len(occurrences) and occurrences[i + 1] == occurrences[i] + len(pattern):
            count += 1
            i += 1
        occurrence_count[start_index] = count
        i += 1
